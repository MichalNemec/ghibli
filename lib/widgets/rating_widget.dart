...
class RatingWidget extends StatefulWidget {
  ...
  /// The current rating value (1–[kMaxRating]), or null if unrated
  final int? rating;
...
class _RatingWidgetState extends State<RatingWidget> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisSize: .min,
      children: List.generate(kMaxRating, (i) {
        final starIndex = i + 1;
...
class FilmRatingTile extends ConsumerWidget {
  /// Creates a tile for the film with [filmId]
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final favorite = ref.watch(favoritesProvider)[filmId];
    ...
    return Row(
      ...
      children: [
        IconButton(
          ...
          icon: Icon(
            favorite?.isFavorite == true ? Icons.favorite_rounded : Icons.favorite_border_rounded,
            color: favorite?.isFavorite == true ? Colors.red : null,
          ),
          onPressed: () async {
